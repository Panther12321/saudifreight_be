frappe.pages["naqil-admin"].on_page_load = function (wrapper) {
  ...
  const reviewApplicant = (organization, decision, reason) => {
    frappe.call({
      method: "naqil.api.review_carrier_applicant",
      args: { organization, decision, reason },
      freeze: true,
      freeze_message: "جارٍ حفظ القرار...",
    }).then(() => {
      frappe.show_alert({ message: decision === "approve" ? "تمت الموافقة على المتقدم" : "تم رفض الطلب", indicator: decision === "approve" ? "green" : "red" });
      renderApplicants();
    });
  };

  const renderApplicant = (organization) => {
    content.html("<p>جارٍ تحميل ملف المتقدم...</p>");
    frappe.call({ method: "naqil.api.get_carrier_applicant", args: { organization } }).then((response) => {
      ...
      content.html(`
        ...
        <section class="naqil-detail-card"><h4>بيانات المتقدم</h4><dl class="naqil-details-grid">
          ...
          <div><dt>الهوية</dt><dd>${escapeHtml(applicant.identity_number)}</dd></div><div><dt>انتهاء الهوية</dt><dd>${displayDate(applicant.identity_expiry_date)}</dd></div>
          <div><dt>رخصة النقل</dt><dd>${escapeHtml(applicant.transport_license_number)}</dd></div><div><dt>انتهاء الرخصة</dt><dd>${displayDate(applicant.transport_license_expiry_date)}</dd></div>
